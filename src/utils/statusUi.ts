...
export const getStatusColor = (status?: string | null): MuiStatusColor => {
  ...
  if (["aprobado", "aprobada", "autorizado", "autorizada", "cerrado", "cerrada", "recibido", "recibida"].includes(value)) {
    return "success";
  }
  if (["pendiente", "pendiente_aprobacion"].includes(value)) return "warning";
