...
export default function Telas() {
  ...
  const [form, setForm] = useState<TelaForm>({
    id: null,
    nombre: "",
    descripcion: "",
  });
  ...
  const cargar = async () => {
    try {
      const resp = await api.get("/telas");
      setRows(resp.data);
    } catch (error) {
      Swal.fire("Error", "No se pudieron cargar telas", "error");
    }
  };
  ...
  const guardar = async () => {
    try {
      ...
      if (editar && form.id != null) {
        await api.patch(`/telas/${form.id}`, payload);
        Swal.fire("Actualizado", "Tela modificada", "success");
      } else {
        await api.post("/telas", payload);
        Swal.fire("Guardado", "Tela creada", "success");
      }
  ...
  const eliminar = useCallback(async (row: any) => {
    ...
    try {
      await api.delete(`/telas/${row.id}`);
      Swal.fire("Eliminado", "Tela borrada", "success");
      cargar();
    } catch (error) {
  ...
  return (
    <Paper sx={{ p: 3, height: "100%" }}>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="sm">
        <DialogTitle>{editar ? "Editar tela" : "Nueva tela"}</DialogTitle>
