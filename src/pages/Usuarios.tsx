...
export default function Usuarios() {
  ...
  const guardar = async () => {
    if (!nombre.trim() || !usuario.trim() || !correo.trim() || (!editing && !password.trim())) {
      Swal.fire("Validación", "Nombre, usuario, correo y contraseña (para nuevos) son obligatorios", "info");
      return;
    }
    if (!rolId) {
      Swal.fire("Validación", "Selecciona un rol", "info");
      return;
    }
    if (!bodegaId) {
      Swal.fire("Validación", "Selecciona una bodega", "info");
      return;
    }
    const payload: any = {
      nombre: nombre.trim(),
